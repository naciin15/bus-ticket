...
if (!defined('LOGIN_TIMEOUT')) define('LOGIN_TIMEOUT', 300);
...
try {
    ...
    if ($search) {
        $sql .= " AND (bus_number LIKE ? OR operator LIKE ? OR departure_city LIKE ? OR arrival_city LIKE ?)";
        $params[] = "%$search%";
        $params[] = "%$search%";
        $params[] = "%$search%";
        $params[] = "%$search%";
    ...
    $sql .= " ORDER BY date DESC, departure_time ASC";
    
    // Count total buses for pagination
    $count_sql = "SELECT COUNT(*) FROM ($sql) AS total";
    $count_stmt = $pdo->prepare($count_sql);
    $count_stmt->execute($params);
    $total_buses = $count_stmt->fetchColumn();
    
    // Pagination
    $per_page = 10;
    $total_pages = ceil($total_buses / $per_page);
    $page = $_GET['page'] ?? 1;
...
<head>
    ...
    <title>Manage Buses - LineBus Admin</title>
        ...
        <main class="main-content">
                ...
                <form method="GET" class="filter-form">
                    ...
                    <div class="filter-group">
                        ...
                        <input type="text" id="search" name="search" class="form-control" 
                               value="<?= htmlspecialchars($search) ?>" placeholder="Bus #, Operator, Route">
                    </div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    <?php if (empty($buses)): ?>
                        <tr>
                            <td colspan="8" style="text-align: center; padding: 2rem;">No buses found</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($buses as $bus): ?>
                            <tr>
                                <td><?= htmlspecialchars($bus['bus_number']) ?></td>
                                <td><?= htmlspecialchars($bus['operator']) ?></td>
                                <td>
                                    <?= htmlspecialchars($bus['departure_city']) ?> → <?= htmlspecialchars($bus['arrival_city']) ?>
                                </td>
                                <td>
                                    <div><?= date('M j, Y', strtotime($bus['date'])) ?></div>
                                    <small><?= date('H:i', strtotime($bus['departure_time'])) ?> - <?= date('H:i', strtotime($bus['arrival_time'])) ?></small>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div style="display: flex; gap: 0.5rem;">
                                        <a href="bus_details.php?id=<?= $bus['id'] ?>" class="action-btn view">
                                            <i class="fas fa-eye"></i> View
                                        </a>
                                        <a href="edit_bus.php?id=<?= $bus['id'] ?>" class="action-btn edit">
                                            <i class="fas fa-edit"></i> Edit
                                        </a>
                                        <a href="delete_bus.php?id=<?= $bus['id'] ?>" class="action-btn delete" onclick="return confirm('Are you sure you want to delete this bus?')">
